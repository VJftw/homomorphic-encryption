...
func TestCompute(t *testing.T) {
	...
	Convey("Should perform basic mathematical operations", t, func() {
		...
		So(calculator.Compute("5 % 2", make(map[string]string)),
			ShouldEqual,
			"1",
		)
		So(calculator.Compute("4 / 2", make(map[string]string)),
			ShouldEqual,
			"2",
		)
		So(calculator.Compute("13 $ 4", make(map[string]string)),
			ShouldEqual,
			"1",
		)
	})

	Convey("Should perform advanced mathematical operations", t, func() {
		So(calculator.Compute("13 & 4,3", make(map[string]string)),
			ShouldEqual,
			"1",
		)
	})
